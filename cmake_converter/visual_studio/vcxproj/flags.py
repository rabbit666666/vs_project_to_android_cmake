...
class CPPFlags(Flags):
    ...
    def __init__(self):
        ...
        self.flags_handlers = OrderedDict([
            ...
            ('CppLanguageStandard', self.__set_language_standard),
            ...
            ('IgnoreSpecificDefaultLibraries', self.__set_target_ignore_specific_default_libraries),
            ...
            ('WholeArchiveBegin', self.__set_whole_link),
        ])
    ...
    def set_defines(context, defines_node):
        ...
        for define in defines_node.text.split(";"):
            define = define.strip()
            if define.find('$(AndroidAPILevelNumber)') != -1:
                continue
    ...
    def __set_precompiled_header(context, flag_name, node):
        ...
        :param node:
        """
        #todo: 需要实现pch, tengjiaozhao 2020/8/30
        is_android = context.is_android
        del context, flag_name
        if is_android or isinstance(node, NodeStub):
            return {}   # default pass
    ...
    @staticmethod
    def __set_whole_link(context, flag_name, node):
        flag_values = {
            'true': {cl_flags: ' -Wl,--whole-archive'},
            'false': {cl_flags: '-Wl,--no-whole-archive'},
            default_value: {}
        }
    ...
    def __set_debug_information_format(context, flag_name, node):
        ...
        """
        if context.is_android or (context.file_contexts is None and node.text == ''):  # if file context ignore default
            return {}
    ...
    def __set_exception_handling(context, flag_name, node):
        ...
        if is_android:
            flag_values = {
                'Disabled': {cl_flags: '-fno-exceptions'},
                'Enabled': {cl_flags: '-fexceptions'},
                default_value: {}
            }
        else:
            flag_values = {
